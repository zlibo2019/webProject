                ...
                <div class="right-content">
                    <div class="table-list" v-for="(item, index) in tableListData" :key="index" v-if="tableListData.length > 0">
                        <div class="empty-box" v-for="(todo, idx) in item.detail" :key="idx">
                            <h5>班级教室：{{item.student_name}}课表<span>(此课表适用时间：{{todo.dateRange}}）</span></h5>
                            <div class="table-box">
	...
	export default {
		...
		data() {
			return {
                ...
                isShowTeacher:sessionStorage.getItem('showTeacherClass')?true:false,//是否显示任课教师
		...
		methods: {
			...
			resizeLayout() {
				setTimeout(() => {
					...
					let ttH = $('.class-timetable .common-top').height();
					let rtH = $('.class-timetable .right-top').height();
					let pbH = $('.class-timetable .pagination-box').height();
					let tblH = icH - ttH  - 20;
					$('.class-timetable .content-box').height(tblH)
					$('.class-timetable .el-tree').height(tblH - rtH -62)
					$('.class-timetable .right-content').height(tblH - rtH -pbH)
            ...
            getTableList() {
                ...
                let params = {
                    "student":[{"id":4,"name":"张三"}],//this.checkedKeys,
                ...
                this.$api.DataCount.getTimetablesStudent('post', params).then( res => {
            ...
            showTeacher(val) {
                ...
                if(val){
                    sessionStorage.setItem('showTeacherClass',1)
                }else{
                    sessionStorage.removeItem('showTeacherClass')
                }
            },
            /**
			 *  @function切换当前页的功能
