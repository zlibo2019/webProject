                ...
                <div class="right-content">
                    <div class="table-list" v-for="(item, index) in tableListData" :key="index" v-if="tableListData.length > 0">
                        <div class="empty-box" v-for="(todo, idx) in item.detail" :key="idx">
                            <h5>班级教室：{{item.classroom_name}}课表<span>(此课表适用时间：{{todo.dateRange}}）</span></h5>
                            <div class="table-box">
	...
	export default {
		...
		data() {
			return {
                ...
                isShowTeacher:sessionStorage.getItem('showTeacherClassroom')?true:false,//是否显示任课教师
		...
		methods: {
			...
			resizeLayout() {
				...
				setTimeout(() => {
					...
					let ttH = $('.place-timetable .common-top').height();
					let rtH = $('.place-timetable .right-top').height();
					let pbH = $('.place-timetable .pagination-box').height();
					let tblH = icH - ttH  - 20;
					$('.place-timetable .content-box').height(tblH)
					$('.place-timetable .el-tree').height(tblH - rtH -30)
					$('.place-timetable .right-content').height(tblH - rtH -pbH)
            ...
            getTableList() {
                ...
                this.$api.DataCount.getTimetablesClass('post', params).then( res => {
            ...
            showTeacher(val) {
                ...
                if(val){
                    sessionStorage.setItem('showTeacherClassroom',1)
                }else{
                    sessionStorage.removeItem('showTeacherClassroom')
                }
            },
            /**
			 *  @function切换当前页的功能
