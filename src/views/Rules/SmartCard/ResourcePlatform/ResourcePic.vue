<template>
    <div class="resource-pic">
		<div class="resource-top">
			<ul>
				<li v-for="(item, index) in navData" @click="navClick(item)" :key="index" class="clearfix" :class="{active:item.value == 0}">
			...
			<div class="search-box">
				...
				<Button btnClass="custom8" :iconLeft="true" :data="btn1Data" btnFontSize="12px" title="上传图片" @click="uploadClick"></Button>
			</div>
		</div>
				...
				<el-col :xs="12" :sm="8" :md="6" :lg="4" :xl="4" v-for="(item, index) in labelData" :key="index"  >
					<div class="inner" style="cursor: pointer;">
						<div @click="ViewDetail(item)">
							<div class="top">
								<img :src="item.pictures.length>0?uploadPicUrl+item.pictures[0].cdn_name+'?imageView2/1/w/246/h/184':''" :onerror="defaultErrImg">
								<span v-if="item.pictures.length>0">{{item.pictures.length}}</span>
							</div>
							<ul class="mid" :style="item.pictures.length>1?'padding-bottom:0':'padding-bottom:25%'">
                                <li v-for="(img, imgIdx) in item.pictures" :key="imgIdx" v-if="imgIdx != 0">
                                    <img :src="uploadPicUrl+img.cdn_name+'?imageView2/1/w/100/h/75'" :onerror="defaultErrImg">
                                </li>
							</ul>
							<h5>{{item.label_name}}</h5>
						</div>
						<p>{{item.creatName}}</p>
						<el-checkbox v-model="item.is_share" disabled v-if="item.is_system == 0 && item.other_share == 0">共享</el-checkbox>
						<Button btnClass="custom8" v-if="item.is_system == 0 && item.other_share == 0" btnWidth="60%" btnHeight="30px" btnFontSize="12px" title="编辑" @click="changeLabels(item)"></Button>
	...
	export default {
		...
		methods: {
			...
			resizeLayout() {
				setTimeout(() => {
					...
					let ttH = $('.resource-pic > .resource-top').height();
					let tblH = icH - stH - ttH -10;
					console.log(66, icH, stH, ttH)
					$('.resource-pic .label-list').height(tblH)
				}, 50)
			},
			/**
			 *  @function: tab点击
				...
				this.$refs['labelForm'].validate((valid) => {
					if (valid) {
                        ...
                        let params = {
                            ...
                            resource_type:1,    //资源类型（1:图片；2:视频；3:文档）
			...
			ViewDetail(item){
				...
				let params = {
					...
					other_share:item.other_share,//是否他人共享
					label_name:item.label_name,//资源标签名称
					creatName:item.user_id//创建人
				}
				this.$router.push({path: '/PicListDetail', query: params})
			},
			/**
			 *  @function: 编辑资源库按钮点击
			 *  @author: cs
			 ...
			 */
			delResource(){
				console.log('删除资源标签',this.checkedResource)
				this.$confirm('这个资源库内所有图片都将被删除！同时删除班牌正在使用的资源！确定删除吗？', '提示', {
		...
		mounted() {
         ...
         created(){
            ...
            console.log(1111111,localStorage.getItem('userId'));
