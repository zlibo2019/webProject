<template>
    <div class="pic-list-detail">
		<div class="detail-top">
			<div class="left">
					<el-input v-model="inputVal" maxlength="20" clearable ref="labelName" size="small" placeholder="请输入资源库名称" style="width:150px;" v-show="isInput"></el-input>
					<h4 v-show="!isInput">
						{{resourceObj.label_name}}
						<span class="count">{{resourceObj.count}}</span>
						<span>{{resourceObj.creatName}}创建人</span>
					</h4>
			...
			<div class="right">	 
				<Button btnClass="custom8" :iconLeft="true" v-show="!modalShow && resourceObj.other_share == 0" :data="btn1Data" btnFontSize="12px" title="上传图片" @click="uploadClick"></Button>
				...
				<Button btnClass="custom6"  v-show="modalShow"  btnFontSize="12px" title="下载" @click="downResource(checkedArr)"></Button>
				<Button btnClass="custom12" v-show="modalShow && resourceObj.other_share == 0"  btnFontSize="12px" title="删除" @click="delResouce({})"></Button>
				<Button btnClass="custom8"  v-show="modalShow && isUse == 0"  btnFontSize="12px" title="发布图片" @click="publishClick"></Button>
				<Button btnClass="custom6"  v-show="modalShow && isUse == 1"  btnFontSize="12px" title="取消发布" @click="cancelUse"></Button>
		...
		<el-main class="label-list">
			<el-row :gutter="20" v-show="isUse == 0">
				<el-col :xs="12" :sm="8" :md="6" :lg="4" :xl="4" v-for="(item, index) in labelData" :key="index"  >
					<div class="inner">
						<div @click="ViewDetail(index)">
							<div class="top">
                                <img :src="uploadPicUrl+item.cdn_name+'?imageView2/1/w/100/h/75'" :onerror="defaultErrImg">
								<!-- <img :src="item.cdn_name" :onerror="defaultErrImg"> -->
							</div>
						...
						<div class="button-box" v-show="!modalShow">
							...
							<Button btnClass="custom8"  btnHeight="30px" btnFontSize="12px" title="发布图片" @click="publishClick(item)"></Button>
						</div>
						<div class="hover-box">
                            ...
                            <button class="btn" @click="downResource([item])"><i class="iconfont icon-xiazai"></i>下载</button></a>
						</div>
					</div>
				...
				<el-col :xs="12" :sm="8" :md="6" :lg="4" :xl="4" v-for="(item, index) in labelData" :key="index"  >
					<div class="inner">
						...
						<div class="button-box">
							...
							btnFontSize="12px" title="取消发布" @click="cancelUse(item)"></Button>
						</div>
						<div class="hover-box">
							<button class="btn" v-if="resourceObj.other_share == 0" @click="editClick(item,index)"><i class="iconfont icon-xiugai"></i>修改</button>
							<button class="btn" v-if="resourceObj.other_share == 0" @click="delResouce(item,index)"><i class="iconfont icon-chuyidong"></i>删除</button>
							<button class="btn" @click="downResource([item])"><i class="iconfont icon-xiazai"></i>下载</button>
						</div>
					</div>
				</el-col>
			</el-row>
			<div class="modal-bot" v-show="modalShow">
				...
				<div class="right">
					<Button btnClass="custom8"  v-show="isUse == 0"  btnFontSize="12px" title="发布图片" @click="publishClick"></Button>
					<Button btnClass="custom6"  v-show="isUse == 1"  btnFontSize="12px" title="取消发布" @click="cancelUse"></Button>
					<Button btnClass="custom12" btnFontSize="12px" v-show="resourceObj.other_share == 0" title="删除" @click="delResouce({})"></Button>
		...
		<!-- 预览图片 -->
		<el-dialog  :visible.sync="viewDetailDialog" v-if="viewDetailDialog" width="900px" center class="show-detail">
			<div slot="title" class="dialog-header">
				<div class="left">
					<div class="source-name">
						<el-input v-model="sourceVal" clearable :ref="'source'+currentNum" size="small" placeholder="请输入资源名称" style="width:150px;" v-show="isSource"></el-input>
						<dd v-show="!isSource">{{labelData[currentNum-1].resource_name}}</dd>
						<span>{{labelData[currentNum-1].uploadDate}}</span>
					</div>
					<Button v-show="isSource" title="保存" btnClass="custom7" btnFontSize="12px" btnHeight="3.5vh" @click="saveEditSourceName"></Button>
                    <Button v-show="isSource" title="取消" btnClass="custom7" btnFontSize="12px" btnHeight="3.5vh" @click="cancelEditSourceName"></Button>
				</div>
				<div class="right">
					<button class="btn" v-show="!isSource" @click="editResouceName"><i class="iconfont icon-xiugai"></i>修改</button>
					<button class="btn" @click="delResouce(labelData[currentNum-1],currentNum-1)"><i class="iconfont icon-chuyidong"></i>删除</button>
					<button class="btn" @click="downResource([item])"><i class="iconfont icon-xiazai"></i>下载</button>
					<Button title="发布到班牌" v-show="isUse == 0" btnClass="custom8" btnFontSize="14px" btnHeight="40px" @click="publishClick(labelData[currentNum-1])"></Button>
					<Button title="取消发布" v-show="isUse == 1" btnClass="custom6" btnFontSize="14px" btnHeight="40px" @click="cancelUse(labelData[currentNum-1])"></Button>
				</div>
			</div>
			 <div class="swiper-container gallery-top">
				<div class="swiper-wrapper">
					<div class="swiper-slide" v-for="(item, index) in labelData" :key="index">
                        <img :src="uploadPicUrl+item.cdn_name+'?imageView2/1/w/580'" :onerror="defaultErrImg">
					</div>
				</div>
			</div>
			...
			<div class="swiper-container gallery-thumbs">
				<div class="swiper-wrapper">
					<div class="swiper-slide" v-for="(item, index) in labelData" :key="index">
                        <img :src="uploadPicUrl+item.cdn_name+'?imageView2/1/h/88'" :onerror="defaultErrImg">
					</div>
				</div>
			</div>
			<div class="current-count" v-if="labelData.length>0"><span>{{currentNum}}</span>/{{labelData.length}}</div>
        </el-dialog>
 
		<!-- 发布图片 -->
		<el-dialog title="发布图片" :visible.sync="publishDialog" width="900px" center class="publish-dialog">
			<!-- <div class="content"> -->
				<el-row :gutter="10">
		...
		<el-dialog title="上传完成" :visible.sync="uploadSuccessDialog" width="400px" center class="upload-success">
				...
				<Button title="发布图片" btnClass="custom8" btnFontSize="12px" btnHeight="40px"
						@click="choosePlaceDialog = true" v-if="roleNos.indexOf('S01') > -1"></Button>
				<Button title="发布图片" btnClass="custom8" btnFontSize="12px" btnHeight="40px" v-if="roleNos.indexOf('S01') == -1"
						@click="publishResource"></Button>
                <br>
		...
		<!-- 上传图片 -->
		<el-dialog title="上传图片" :visible.sync="uploadDialog" width="650px" center class="upload-dialog">
            <div class="content">
	...
	export default {
		...
		data() {
			return {
				...
				// copyData:'',//复制labelData，选中之后点击取消发布 暂时不需要
				checkedArr:[],//选中的数组
				labelData:[],//文件夹列表
				currentNum:1,//当前第几页
		...
		methods: {
			...
			resizeLayout() {
				setTimeout(() => {
					...
					let ttH = $('.pic-list-detail > .detail-top').height();
					let dmH = $('.pic-list-detail > .detail-mid').height();
					let tblH = icH - ttH - dmH;
					console.log(66, icH, ttH,tblH)
					$('.pic-list-detail .label-list').height(tblH)
				}, 200)
			},
			/**
			 *  @function: 上方搜索
			...
			ViewDetail(index){
				...
				let mySwiper = null;
				setTimeout(() => {
					let galleryThumbs = new Swiper('.show-detail .gallery-thumbs', {
						initialSlide :index,
						spaceBetween: 10,
						slidesPerView: this.labelData.length>6?6:this.labelData.length,
						freeMode: true,
						...
						on: {
							slideChangeTransitionStart: function(){
								//轮播之前执行
								vThis.isSource = false;
								vThis.currentNum = this.activeIndex+1;
								if (this.activeIndex > vThis.labelData.length) {
									vThis.currentNum = vThis.labelData.length;
					...
					let galleryTop = new Swiper('.show-detail .gallery-top', {
						spaceBetween: 10,
						initialSlide :index,
						navigation: {
			 ...
			 */
			back(){
                console.log('返回上一页',localStorage.getItem('userId'))
			...
			batchManage(){
				...
				// this.copyData = JSON.stringify(this.labelData);
			},
			/**
			 *  @function: 发布图片按钮点击
			 *  @author: cs
			...
			cancelUse(item){
				...
				if(!item){
					...
					console.log(112,this.checkedClass);
				}else{
					this.checkedArr = [];
					this.checkedArr.push(item);
				}
				console.log("checkedArr:",this.checkedArr);
				this.$confirm('取消选中图片在所在教室的使用，取消后不可恢复！确定取消吗？', '提示', {
			...
			delResouce(item,index){
				...
				this.$confirm('这个资源将被删除！同时删除班牌正在使用的资源！确定删除吗？', '提示', {
                    ...
                    let params = {
                        ...
                        resource_type:1,//标签类型 1图片 2视频 3文档
			...
			saveEditSourceName(){
                ...
                this.putResource(this.labelData[this.currentNum-1]);
			},
			/**
			 *  @function: 取消保存资源名称
			 *  @author: cs
			...
			closeModal(){
				...
				this.modalShow = false;
				// this.labelData = JSON.parse(this.copyData);
			},
			/**
			 *  @function: 编辑保存资源
			 *  @author: cs
			...
			publishResource(){
                ...
                console.log(params,this.listClassroom);
			...
			downResource(data){
                ...
                for (let i = 0; i < data.length; i++) {
                   this.downloadIamge(this.uploadPicUrl+data[i].cdn_name,data[i].resource_name);
                }
            },
            downloadIamge(imgsrc, name) { 
                ...
                image.onload = function() {
                    ...
                    let b = a.dispatchEvent(event); // 触发a的单击事件IE下使用fireEvent    高级浏览器下使用dispatchEvent 
                    console.log(b)  // true
                };
                image.src = imgsrc;
            },
		...
		mounted() {
            ...
            console.log(112,this.resourceObj);
            // this.downloadVideo('http://res-mal.weds.com.cn/FkOK078k_tk3CF_6EB7QbNsj4wAH',147);
         },
         created(){
